...
public final class NativeBackend: NSObject, AppBackend {
  ...
  /// The definition lives in `AmazonURLs`, shared with the capture.
  public nonisolated static func isSignedInUrl(_ url: URL?) -> Bool {
    AmazonURLs.isSignedIn(url)
  }
  ...
  public func signIn() async -> Bool {
    ...
    // On the signed-out landing page, go straight to Amazon's form: showing
    // the landing page adds a step whose purpose isn't obvious.
