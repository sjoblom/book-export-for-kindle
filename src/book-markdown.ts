...
// The markdown export as a pure function of the metadata and text, apart from
// the file handling in `export-book-markdown.ts`, so the native app renders
// the same document through JavaScriptCore.
...
/**
 * The book's text as it is fit to export.
 *
 * Only what the pipeline's checks would accept: text from this capture's
 * pages, one chunk per page, each with actual text. Rendering `content.json`
 * as-is would print a stale capture or duplicate pages that every
 * completeness check had already discounted.
 *
 ...
 *
 * `content` is either the store as read from `content.json` — whose capture id
 * is checked against the metadata — or bare chunks the caller vouches for as
 * belonging to this capture.
 */
export function exportableChunks(
  metadata: BookMetadata,
  content: ContentStore | ContentChunk[] | null | undefined
): ContentChunk[] {
  const store = Array.isArray(content)
    ? { captureId: metadata.captureId, chunks: content }
    : (content ?? undefined)

  return withCurrentText(selectReusableChunks(store, metadata), metadata)
}

export interface RenderedMarkdown {
  /** The file name to write it under, derived from the title. */
  fileName: string
  markdown: string
}
...
export function renderBookMarkdown(
  ...
  const sections = resolveBookSections(metadata.toc, chunks)
...
${bodies[i]}`
  ...
  return { fileName: filenameFromTitle(title), markdown: output }
}
